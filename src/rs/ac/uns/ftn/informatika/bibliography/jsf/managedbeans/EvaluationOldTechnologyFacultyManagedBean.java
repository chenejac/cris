...
public class EvaluationOldTechnologyFacultyManagedBean extends CRUDManagedBean implements IPickJournalManagedBean{
	...
	public EvaluationOldTechnologyFacultyManagedBean() {
		super();
		init();	
	...
	protected String dispetcher(int condition) {
		String retVal = null;
		retVal = "evaluationOldTechnologyFacultyPage";
		return retVal;
	}
	...
	public String enterPage() {
		...
		return "evaluationOldTechnologyFacultyPage";
	}
...
//		System.out.println(journalControlNUmber);
	...
	public void calculateExplainTable() {
		...
		AbstractCommissionEvaluation commisionTechnology = null;
		CommissionFactory commissionFactory = CommissionFactory.getInstance();
		
		if (commissionID >=500 && commissionID <=500){
			commisionTechnology = commissionFactory.getCommissionEvaluation(commissionID.intValue());
		}
		...
		if(commisionTechnology.getMnoList() != null)
			mnoList.addAll(commisionTechnology.getMnoList());
		List<CommissionDTO> conComs = commisionDAO.getConnectedCommissions(commisionTechnology.getComissionID());
		...
		for (MNO mno : mnoList) {
			...
			if(mno.isSpecial(selectedJournal.getIssn(), 0)){
				if("AMS".equals(mno.getNameMNO()))
					isSpecialMNO.append("AMS (1981-2014:M24) ");
				else {
