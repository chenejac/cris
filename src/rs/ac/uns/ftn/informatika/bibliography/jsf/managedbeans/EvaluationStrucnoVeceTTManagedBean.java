...
import rs.ac.uns.ftn.informatika.bibliography.evaluation.commisions.CommisionStrucnoVeceTTTehnoloskoInzenjerstvo;
...
public class EvaluationStrucnoVeceTTManagedBean extends CRUDManagedBean implements IPickJournalManagedBean{
	...
	public EvaluationStrucnoVeceTTManagedBean() {
		super();
		init();	
	...
	public void init ()
	{
		...
		try {
	        ...
	        //STRUCNO VECE KOMISIJE od 150-200
	        
	        for (CommissionDTO com : commissionList){
	        	if(com.getCommissionId()>=150 && com.getCommissionId()<200)
	...
	protected String dispetcher(int condition) {
		String retVal = null;
		retVal = "evaluationStrucnoVeceTTPage";
		return retVal;
	}
	...
	public String enterPage() {
		...
		return "evaluationStrucnoVeceTTPage";
	}
	...
	public void setCommissionRuleBook() {
		...
		if(commissionID!=null){
			if(commissionID.intValue()>=101 && commissionID.intValue()<150)
				pdfUrl = facesMessages.getMessageFromResourceBundle("evaluation.mainPanel.yearCommissionSelection.commission.rulebook.id"+101);
			else if(commissionID.intValue()>=150 && commissionID.intValue()<200)
				pdfUrl = facesMessages.getMessageFromResourceBundle("evaluation.mainPanel.yearCommissionSelection.commission.rulebook.id"+150);
		}
...
//		System.out.println("metoda evaluate pokrenuta*************************************************************************");
		...
		if (selectedJournal != null && commissionID != null && commission != null && year != null){
			...
			List<ResultMeasureDTO> retVal = ResultEvaluator.getResultTypesVece(record, year, ruleBook, commission);
...
//				System.out.println("Kategorija " + rM.getResultType().getClassId() + " opis " + rM.getResultType().getSomeTerm() + " ocena " + rM.getQuantitativeMeasure());
			...
			if(evaluatedResylts.isEmpty()){
                String [] pom = {facesMessages.getMessageFromResourceBundle("evaluation.evaluationData.notEvaluatedExplain"), facesMessages.getMessageFromResourceBundle("evaluation.evaluationData.notEvaluatedExplain"), "0"};
                evaluatedResylts.add(pom);
            }
...
//		System.out.println(journalControlNUmber);
	...
	public void calculateExplainTable() {
		...
		AbstractCommissionEvaluation commisionStrucnoVece = null;
		CommissionFactory commissionFactory = CommissionFactory.getInstance();
		
		if (commissionID >=101 && commissionID <=200){
			commisionStrucnoVece = commissionFactory.getCommissionEvaluation(commissionID.intValue());
		}
		...
		if(commisionStrucnoVece.getMnoList() != null)
			mnoList.addAll(commisionStrucnoVece.getMnoList());
		List<CommissionDTO> conComs = commissionDAO.getConnectedCommissions(commisionStrucnoVece.getComissionID());
		for (CommissionDTO commissionDTO : conComs) {
			...
			List<CommissionDTO> conComs2 = commissionDAO.getConnectedCommissions(commissionDTO.getCommissionId());
...
//				System.out.println(i +" unete kat " + kljucKategorije +" vrsta "+red[i]);
	...
	public HashMap<Integer, String> getAllTypes(Record record, CommissionDTO commission, Integer startYear, Integer endYear) {
		...
		journalCategories.put("leadingInternationalJournal", new Integer(8));
		...
		for (int i = startYear; i <= endYear; i++) {
			...
			if(classSuperPublicationType == null){
				List<CommissionDTO> conComs = commissionDAO.getConnectedCommissions(commission.getCommissionId());
				for (CommissionDTO commissionDTO : conComs) {
					for (Classification classification : record.getRecordClasses()) {
						if(
...
//							break;
				...
				List<CommissionDTO> conComs1 = commissionDAO.getConnectedCommissions(commission.getCommissionId());
				for (CommissionDTO commissionDTO1 : conComs1) {
					List<CommissionDTO> conComs2 = commissionDAO.getConnectedCommissions(commissionDTO1.getCommissionId());
