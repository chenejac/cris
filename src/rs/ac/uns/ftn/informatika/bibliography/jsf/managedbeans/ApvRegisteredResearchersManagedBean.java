...
public class ApvRegisteredResearchersManagedBean extends CRUDManagedBean {
	...
	public void enterPage(PhaseEvent event){
		...
		if((init == false) || (departmentId != null) || (institutionId != null)){
			...
			INCN = institutionId;
			if(OUCN != null){
				OUCNQuery = personDAO.getInstitutionRecordsQuery(OUCN, "" + Calendar.getInstance().get(Calendar.YEAR) + "-01-01 00:00:00");
			} else 
				OUCNQuery = null;
			if(INCN != null){
				INCNQuery = personDAO.getInstitutionRecordsQuery(INCN, "" + Calendar.getInstance().get(Calendar.YEAR) + "-01-01 00:00:00");
			} else 
				INCNQuery = null;
			enterCRUDPage();
			init = true;
