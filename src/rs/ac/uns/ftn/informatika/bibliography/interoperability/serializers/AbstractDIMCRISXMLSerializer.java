...
//				retVal.add(textTag);
	...
	protected List<XMLTag> getTypes(String mdschema, String element, RecordDTO record) {
		...
		if(record instanceof StudyFinalDocumentDTO){
			...
			for (String language : languages) {
				if(language.equals("en")){
					XMLTag typeTag = new XMLTag(tagName, (record.getControlNumber().equals("(BISIS)134397") || record.getControlNumber().equals("(BISIS)136037") || record.getControlNumber().equals("(BISIS)139351"))?"archWork":"Book");
					typeTag.getAttributes().add(new AttributeValue("mdschema", mdschema));
					typeTag.getAttributes().add(new AttributeValue("element", element));
					typeTag.getAttributes().add(new AttributeValue("lang", "en"));
					retVal.add(typeTag);
				} else if(language.equals("sr")){
					XMLTag typeTag = new XMLTag(tagName, (record.getControlNumber().equals("(BISIS)134397") || record.getControlNumber().equals("(BISIS)136037") || record.getControlNumber().equals("(BISIS)139351"))?"Studija":"Monografija");
					typeTag.getAttributes().add(new AttributeValue("mdschema", mdschema));
					typeTag.getAttributes().add(new AttributeValue("element", element));
					typeTag.getAttributes().add(new AttributeValue("lang", "sr"));
					retVal.add(typeTag);
