 ...
 *
 */
public class ImportBooksFromKNRTask implements Task {
	...
	public ImportBooksFromKNRTask(Connection source) {
		this.source = source;
	}
	...
	public boolean execute() {
		...
		try {
			...
			query
					.append("select ID, ADDOPTEDTITLE, AUTHORS, PUBLISHERNAME, PUBLISHERPLACE, ADDOPTEDPUBLICATIONYEAR, ISBN, UDK from BOOK where CONTROLNUMBER not like '(BISIS)%' AND ADDOPTED=1");
			Statement stmt = source.createStatement();
			PreparedStatement pstmt = source
				.prepareStatement("update BOOK set CONTROLNUMBER=? where ADDOPTEDTITLE=?");
...
//					String udk = rset.getString(8);
					...
					if(authors.trim().length() > 0)
						setAuthorsOrEditors(monographDTO, authors);
	...
	private void setAuthorsOrEditors(MonographDTO monographDTO,
			String authors) throws Exception{
		RecordDAO recDAO = new RecordDAO(new PersonDB());
		boolean editor = true; //false;
//		if(authors.startsWith("EDITOR:")){
...
//		}
		...
		for (AuthorNameDTO authorNameDTO : parsedAuthorsNames) {
			...
			if(appropriateAuthors.size() == 1)
				authorsList.add(appropriateAuthors.get(0));
				...
				authorsList.add(addAuthorDTO);
			} else {
				throw new Exception("Nije jednoznacno: " + authorNameDTO);
			}
		}
		if(editor)
			monographDTO.setEditors(authorsList);
		else {
	...
	private static Log log = LogFactory.getLog(ImportBooksFromKNRTask.class.getName());
