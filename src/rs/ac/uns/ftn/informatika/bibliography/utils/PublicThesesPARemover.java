...
import rs.ac.uns.ftn.informatika.bibliography.jsf.managedbeans.PublicThesesPAManagedBean;
 ...
 */
public class PublicThesesPARemover extends TimerTask{
	...
	public void run() {
		...
		String whereClause = PublicThesesPAManagedBean.PUBLIC_THESES_WHERE_CLAUSE;
			...
			for (Record record : records) {
				...
				if((theses.getReport() != null) && (theses.getPreliminaryTheses() != null)){
					if(time - theses.getReport().getDateModified().getTimeInMillis() > (30l*24*60*60*1000)){
						log.info("Public period finished: " + theses);
...
//							theses.getPreliminaryTheses().setLicense(null);
			...
			if(!message.equals("")){
				PublicThesesPAManagedBean.PUBLIC_THESES_LAST_MODIFICATION_DATE = new Date();
				message = rbAdministration.getString("administration.notification.publicTheses.expired") + message;
				PublicThesesPAManagedBean.publicThesesExpiredMessage(message);
			}
			if(!errorMessage.equals("")){
				errorMessage = rbAdministration.getString("administration.notification.publicTheses.error") + errorMessage;
				PublicThesesPAManagedBean.publicThesesExpiredErrorMessage(errorMessage);
			}
		}
